...
function handleSendChatPhoto(e) {
    const file = e.target.files[0];
    if (file && activeChatUserId) {
        const reader = new FileReader();
        reader.onload = function(evt) {
            if (!chatsDB[activeChatUserId]) chatsDB[activeChatUserId] = [];
            chatsDB[activeChatUserId].push({ from: 'me', text: '📷 Shared a photo', image: evt.target.result });
            syncStorage();
            renderChatView();
        };
        reader.readAsDataURL(file);
...
function handleSaveProfile(e) {
    ...
    me.role = document.getElementById('edit-role').value;
    me.church = document.getElementById('edit-church').value.trim();
    me.bio = document.getElementById('edit-bio').value.trim();
    if (selectedAvatarBase64) me.avatar = selectedAvatarBase64;
